...
enum TealiumConfiguration {
    static let account = "services-kiyoshi-amano"
    static let profile = "ios-kiyotaro"
    static let environment = "dev"
    static let dataSourceKey = "mxcbxh"
}
...
class TealiumHelper {
    ...
    private init() {
        ...
        config.geofenceFileName = "geoKiyo"
        config.useHighAccuracy = true
        config.updateDistance = 300.0
        config.desiredAccuracy = .nearestTenMeters
        config.geofenceTrackingEnabled = true
        ...
        tealium = Tealium(config: config) { _ in
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                if #available(iOS 14, *) {
                    self.tealium?.location?.requestTemporaryFullAccuracyAuthorization(purposeKey: "NearStore1") // key must match what is in Info.plist
                }
            }
